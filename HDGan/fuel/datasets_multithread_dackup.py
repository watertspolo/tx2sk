...
class Dataset(object):
    ...
    def get_data(self, data_dir):
        ...
        """filename, キャプション, embedding のパスを取得"""
    ...
    def __getitem__(self, index):
        ...
        filenames = [(self.filenames[i].decode())[15:] for i in current_ids]
        fake_filenames = [(self.filenames[i].decode())[15:] for i in fake_ids]

        print(filenames)

        sampled_images = self.images(filenames)
        # print(self.images(filenames))
...
class COCODataset():
    def __init__(self, data_dir, img_size, batch_size, num_embed, mode='train', threads=1, drop_last=True):
